...
var mapContants = {
    ...
    '西方海域戦線 カレー洋': '2015年夏季活动/主作战#E-5',
    'ソロモン海東部海域': '2015年夏季活动/主作战#E-6',
    'FS方面海域': '2015年夏季活动/主作战#E-7',
}
...
function requestData(map_name) {
    $.getJSON('/drop/map/' + map_name + ".json", function(data) {
        ...
        $.each(data.result, function(key, val) {
            var table = $('table[data-eneny-name="' + val.name + '"]');
        ...
        $('.fixed-table-toolbar > div:nth-of-type(2)')
            .append("<a class='btn btn-default' href='http://zh.kcwiki.moe/wiki/" +
                mapContants[map_name] + "' target='_blank'>查看百科</a>");
...
function detailFormatter(index, row) {
    ...
    if (row.mapLvCount[1] + row.mapLvCount[2] + row.mapLvCount[3] > 0) {
        ...
        html.push("<div class='table-like'><div><span>甲难度</span><span>" +
            row.mapLvCount[3] + " (" +
            (row.mapLvCount[3] * 100 / row.totalCount).toFixed(2) +
