...
$common_maps = [(11..16).to_a, (21..25).to_a, (31..35).to_a].flatten #, (41..45).to_a, (51..55).to_a, (61..65).to_a, 71].flatten
$event_maps = []

def staticify_drop_map(map_id)
  levels = map_id > 100 ? [3, 2, 1] : [0]
  table = map_id > 100 ? DropRecordSummer2017 : DropRecord
  table_name = map_id > 100 ? "drop_records_summer2017" : "drop_records"
  levels.each do |level_no|
    KanColleConstant.map[map_id][:cells].each do |cell|
      ...
      Sinatra::KVDataHelper.set_kv_data("wdrop_map_#{map_id}_#{cell[:point]}#{level_no > 0 ? "-#{level_no}" : ""}-SA", json_obj.to_json)
      ...
      Sinatra::KVDataHelper.set_kv_data("wdrop_map_#{map_id}_#{cell[:point]}#{level_no > 0 ? "-#{level_no}" : ""}-SAB", json_obj.to_json)
    end
  end
end

def staticify_drop_ship()
  table = [DropRecord, DropRecordSummer2017]
  table_name = ["drop_records", "drop_records_summer2017"]
  ...
  ['S', 'A', 'B', 'SA', 'SAB'].each do |rank|
    ...
    [$common_maps, $event_maps].flatten.each do |map|
      ...
      levels.each do |level|
        KanColleConstant.map[map][:cells].each do |cell|
          drop_map_data["#{map / 10}-#{map % 10}-#{cell[:point]}#{cell[:boss] ? "(Boss)" : ""}#{level == 0 ? "" : (level == 3 ? "-甲" : (level == 2 ? "-乙" : "-丙"))}"] = JSON.parse Sinatra::KVDataHelper.get_kv_data("wdrop_map_#{map}_#{cell[:point]}#{level > 0 ? "-#{level}" : ""}-#{rank}")
        end
      end
    end
    ...
    ship_set.each do |ship|
      ...
      Sinatra::KVDataHelper.set_kv_data("wdrop_ship_#{ship_id}-#{rank}", json_obj.to_json)
    end
  end
end
